function [extra_space, rms_data] = extra_disturbance_study_wrong( ...
    Fs, n_photons, options)
%extra_disturbance_study_wrong Wrong known extra disturbance study
% 
%   extra_disturbance_study_wrong(Fs, n_photons) gives the rms value of the
%   error for the controller when the known extra disurbances during the design
%   phase are wrong. The flux noise is determined by the `n_photons` recieved
%   per timestep. Controller frequency is given by `Fs` Hz.
% 
%   extra_disturbance_study_wrong(___, name, value) specifies the optional and
...
%
    ...
    arguments
        ...
        options.extra_type      (1, 1) string  = "none"
    ...
    frd_extra = cell(1, n_extra_space);
    for i_extra_space = 1:n_extra_space
        switch options.extra_type
            ...
            case "narrowband"
                % Use worst case for frequency and bandwidth
                ts_extra = ...
        ...
        [psd, freq] = pwelch(ts_extra.Data, [], [], ...
            TMT_disturbance.w / 2 / pi, Fs_sim);
        frd_extra{i_extra_space} = frd(sqrt(psd), freq * 2 * pi, 1 / Fs_sim);
    end

    %% Design controllers ------------------------------------------------------
    empty_cells = cell(1, n_extra_space);
    controllers = struct(...
        "tip" , empty_cells, ...
        "tilt", empty_cells  ...
    );
    ...
    for i_extra_space = 1:n_extra_space
        for DoF = ["tip", "tilt"]
            for type = ["int", "data"]
                controllers(i_extra_space).(DoF).(type) = ...
                    VibrationController(type, ...
                        "Fs"     , Fs            , ...
                        "tau_lag", tau_lag       , ...
                        "solver" , options.solver  ...
                    );
                controllers(i_extra_space).(DoF).(type).set_disturbance_frd( ...
                    TMT_disturbance.get_frd_model(DoF) + ...
                    frd_extra{i_extra_space});
                controllers(i_extra_space).(DoF).(type).set_flux_noise_rms( ...
                    utils.get_flux_noise_rms(n_photons, ...
                        "readout_noise", options.readout_noise));
                if type == "int"
                    controllers(i_extra_space).(DoF).(type).update_parameter(...
                ...
                controllers(i_extra_space).(DoF).(type).design(false);
            end
        end
    end
