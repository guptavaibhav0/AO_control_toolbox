function varying_atmospheric_adaptive(Fs, n_photons, options)
%varying_atmospheric_adaptive Night variation study for adaptive controller
% 
%   varying_atmospheric_adaptive(Fs, n_photons) gives rms value of the error for
%   the adaptive controller over all night variations. The flux noise is
%   determined by the `n_photons` recieved per timestep. Controller
%   frequency is given by `Fs` Hz.
% 
%   varying_atmospheric_adaptive(___, name, value) specifies the optional and 
...
%     |                | `varying_atmospheric_adaptive/%d_Hz/%d_photons.mat`   |
%     |                |   Default: true                                       |
%     |----------------|-------------------------------------------------------|
...
% 
    ...
    parent_folder = fullfile( ...
        datafolder, ...
        "varying_atmospheric_adaptive", ...
        sprintf("%04d_Hz", Fs));
    save_filename = fullfile( ...
        parent_folder, ...
        sprintf("%04d_photons.mat", n_photons));
    tmp_save_filename = fullfile( ...
        parent_folder, ...
        sprintf("%04d_photons_tmp.mat", n_photons));
    ...
    if isfile(tmp_save_filename)
        load(tmp_save_filename, ...
            "rms_data", ...
            "last_variations_space");
    end

    for i_variations_space = 1:n_variations_space
        if i_variations_space <= last_variations_space
            if print_progress
                send(progressQueue, 1);
            end
            continue;
        end
        last_variations_space = i_variations_space;
